...
export const getParkingAreas = async (): Promise<ParkingServiceResult<ParkingAreaModel[]>> => {
  try {
    ...
    return { ok: false, error: toServiceError(error, 'Failed to fetch parking areas') };
  }
};

export const getSlotsByArea = async (areaId: string): Promise<ParkingServiceResult<ParkingSlotModel[]>> => {
...
export const listenToSlotUpdates = (areaId: string, callback: SlotUpdatesCallback): UnsubscribeListener => {
  const normalizedAreaId = areaId.trim();

  if (!normalizedAreaId || typeof callback !== 'function') {
    return () => undefined;
  }
  ...
  const unsubscribe = onSnapshot(
    ...
    (snapshot) => {
      ...
      snapshot.forEach((item) => {
        try {
          ...
          console.error('[admin parkingService] skipped invalid slot document', {
            id: item.id,
            error: mappingError,
          });
        }
    ...
    (error) => {
      console.error('[admin parkingService] listenToSlotUpdates failed', toServiceError(error, 'Failed to listen for slot updates'));
    },
  );
...
export const updateSlotStatus = async (
  slotId: string,
  status: SlotStatus,
): Promise<ParkingServiceResult<ParkingSlotModel>> => {
